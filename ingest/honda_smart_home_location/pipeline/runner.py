...
def run_pipeline(mode: str = "dev", input_files: List[str] = []):
    ...
    pipeline_config = os.path.join(
        config_dir, "pipeline_config_honda_smart_home_location.yml"
    )
    storage_config_filename = f"storage_config_honda_smart_home_location.yml"
    ...
    if len(input_files) == 0:
        input_files = [os.path.join(data_dir)]
